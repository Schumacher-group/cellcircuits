...
def stochastic_transient_injury(x, t, dt):
    regular_derivatives = mF_M_rates(x, t)
    noise_term = np.random.normal(0, 1) * np.sqrt(dt) * sigma if t <= 2 else 0
    dmFdt = regular_derivatives[0] * dt  # deterministic update for mF
    dMdt = (regular_derivatives[1] + (I_mean if t <= 2 else 0)) * dt + noise_term
...
for i in range(1, num_steps):
    x_stochastic[i] = x_stochastic[i - 1] + stochastic_transient_injury(x_stochastic[i - 1], t[i - 1], dt)
...
# COUNT NUMBER OF HEALING AND NUMBER OF FIBROSIS OUTCOMES - NEW CORRECTED CODE
