...
#bar chart results for fibrosis and healing
...
for end_point in final_end_points:
    if end_point[1] < np.interp(end_point[0], separatrix_left[:, 0], separatrix_left[:, 0]):
        healing_count += 1
    else:
        fibrosis_count += 1
...
# bar chart results different sigma levels 
# Define different noise levels
sigma_values = [0.5 * A_0, 1 * A_0, 2 * A_0]
results = []
...
for sigma in sigma_values:
    ...
    # Run the stochastic simulation multiple times for each sigma
    for _ in range(num_simulations):
        x_stochastic = np.zeros((num_steps, len(x_in)))
        x_stochastic[0] = x_in
        ...
        final_end_point = (x_stochastic[-1][0], x_stochastic[-1][1])
        
        # Determine if the final end point is healing or fibrosis
        if final_end_point[1] < np.interp(final_end_point[0], separatrix_left[:, 0], separatrix_left[:, 0]):
            healing_count += 1
        else:
            fibrosis_count += 1
    
    # Store the results for this sigma value
    results.append((sigma, healing_count, fibrosis_count))

# Plot the results
fig, ax = plt.subplots()

# Extract the data for plotting
sigma_labels = [f'{sigma/A_0:.1f} A_0' for sigma in sigma_values]
healing_counts = [result[1] for result in results]
fibrosis_counts = [result[2] for result in results]

# Define the bar width and positions
bar_width = 0.35
index = np.arange(len(sigma_values))

# Plot healing and fibrosis counts side by side
bar1 = ax.bar(index, healing_counts, bar_width, label='Healing (0)', color='blue')
bar2 = ax.bar(index + bar_width, fibrosis_counts, bar_width, label='Fibrosis (1)', color='red')

# Set plot attributes
ax.set_xlabel('Sigma Levels')
ax.set_ylabel('Count')
ax.set_title('Number of Healing and Fibrosis Outcomes at Different Noise Levels')
